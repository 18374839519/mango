...
import com.louis.mango.admin.security.utils.model.GrantedAuthorityImpl;
import com.louis.mango.admin.security.utils.model.JwtUserDetails;
...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        SysUser user = sysUserServiceImpl.selectByUserName(username);
        if (user == null) {
            throw new BaseException(HttpStatus.SC_NOT_FIND, "该用户不存在");
        }
        // 用户权限列表，根据用户拥有的权限标识与如 @PreAuthorize("hasAuthority('sys:menu:view')") 标注的接口对比，决定是否可以调用接口
        Set<String> permissions = sysMenuServiceImpl.selectPermissionsByUserName(user.getName());
